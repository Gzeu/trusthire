import { isBlacklisted } from '@/lib/blacklistCache';
import {
  AssessmentInput,
  RedFlag,
  ScoreBreakdown,
...
export async function calculateScores(
  ...
  const email = input.recruiter.emailReceived?.toLowerCase().trim();
  const emailDomain = email?.split('@')[1]?.trim();

  if (email) {
    const emailHit = await isBlacklisted(email);
    if (emailHit.hit) {
      return {
        identityConfidence: 0,
        employerLegitimacy: 0,
        processLegitimacy: 0,
        technicalSafety: 0,
      };
    }
  ...
  if (emailDomain) {
    const domainHit = await isBlacklisted(emailDomain);
    if (domainHit.hit) {
      return {
        identityConfidence: 0,
        employerLegitimacy: 0,
  ...
  for (const dc of domainChecks) {
    const domainHit = await isBlacklisted(dc.domain.toLowerCase().trim());
    if (domainHit.hit) {
      return {
        identityConfidence: 0,
        employerLegitimacy: 0,
  ...
  for (const scan of repoScans) {
    const repoHit = await isBlacklisted(scan.repoUrl.toLowerCase().trim());
    if (repoHit.hit) {
      return {
        identityConfidence: 0,
        employerLegitimacy: 0,
  ...
  if (recruiter.emailReceived) {
    const recruiterEmail = recruiter.emailReceived.toLowerCase();
    const company = recruiter.claimedCompany.toLowerCase().replace(/\s+/g, '');
    const domain = recruiterEmail.split('@')[1] || '';
    ...
    if (/20(24|25|26|27)/.test(recruiterEmail)) identity -= 8;
    if (recruiterEmail.includes('temp') || recruiterEmail.includes('test') || recruiterEmail.includes('verify')) identity -= 10;
    if (recruiterEmail.includes('recruiter') || recruiterEmail.includes('hr')) identity -= 2;
  } else {
    identity -= 4;
  }
  ...
  if (recruiter.connections && recruiter.connections > 0) {
    ...
    const isSeniorRecruiter =
      title.includes('senior') || title.includes('lead') || title.includes('principal') || title.includes('head');
...
export function generateRedFlags(
  ...
  if (job.walletSeedKycRequest) {
    flags.push({
      ...
      explanation:
        'Legitimate employers never ask for crypto wallet access, seed phrases, private keys, or sensitive KYC documents during early recruitment.',
  ...
  if (job.runCodeLocally) {
    flags.push({
      ...
      explanation:
        'Running untrusted code locally is one of the most common delivery methods for info-stealers in fake technical assessments.',
  ...
  if (combinedMessage.includes('telegram') || combinedMessage.includes('whatsapp')) {
    flags.push({
      ...
      explanation:
        'Moving recruiting conversations to Telegram or WhatsApp is common in scam workflows because it reduces accountability.',
      recommendation: 'Keep communication on official channels such as company email or verified LinkedIn accounts.',
    });
  }

  if (
    combinedMessage.includes('google form') ||
    combinedMessage.includes('typeform') ||
    combinedMessage.includes('complete the form')
  ) {
    flags.push({
      category: 'process',
      severity: 'warning',
      ...
      explanation:
        'Scammers often rush targets into forms to collect personal details before trust is established.',
  ...
  for (const scan of repoScans) {
    if (scan.dangerousScripts.length > 0) {
      flags.push({
        ...
        explanation:
          'Lifecycle scripts can execute automatically during installation and are frequently abused for silent payload delivery.',
    ...
    for (const pm of scan.patternMatches.filter((p) => p.severity === 'critical')) {
      flags.push({
        ...
        explanation:
          'This code pattern is strongly associated with code execution, credential theft, or payload download behavior.',
    ...
    if (scan.patternMatches.some((p) => p.pattern.startsWith('suspicious_dependency:'))) {
      flags.push({
        ...
        explanation:
          'The repository depends on a package name that resembles known typosquatting or impersonation behavior.',
...
export function generateMissingEvidence(input: AssessmentInput): string[] {
  ...
  if (!recruiter.recruiterMessages || recruiter.recruiterMessages.length < 50) {
    missing.push('Recruiter conversation history not provided');
  }
...
export function generateWorkflowAdvice(verdict: Verdict, flags: RedFlag[]): WorkflowStep[] {
  ...
  if (hasWallet) {
    steps.push({
      ...
      priority: 'urgent',
      description: 'Immediately stop communication. Never share wallet credentials, seed phrases, private keys, or KYC documents.',
    });
    steps.push({
      action: 'rotate_secrets',
      priority: 'urgent',
      description: 'If anything sensitive was shared, rotate wallet keys, API keys, passwords, session tokens, and SSH credentials immediately.',
    });
  }
  ...
  if (hasRunCode || hasRepoRisk) {
    steps.push({
      ...
      priority: 'urgent',
      description: 'Do not run npm install, pnpm install, pip install, shell scripts, or setup scripts on your local machine.',
    });
    steps.push({
      action: 'use_vm_sandbox',
      priority: 'high',
      description: 'If code review is required, only inspect it inside an isolated VM or disposable sandbox with no access to real credentials.',
    });
    steps.push({
      action: 'report_repository',
      priority: 'high',
      description: 'Report the suspicious repository to GitHub with details about the dangerous scripts or malicious patterns detected.',
    });
  }
  ...
  if (hasVTMalicious) {
    steps.push({
      ...
      priority: 'high',
      description: 'Report the recruiter profile or sender account because the associated infrastructure was flagged by security vendors.',
    });
  }
  ...
  steps.push({
    ...
    priority: 'medium',
    description: 'Request a video call, official company email confirmation, and a public job listing on the company website.',
  });
  steps.push({
    action: 'request_more_proof',
    priority: 'medium',
    description: 'Verify the company through official domains, legal registration records, and real employee profiles.',
  });
  steps.push({
    action: 'safe_to_proceed',
    priority: 'low',
    description: 'Keep a record of all recruiter messages, links, and repositories for auditability.',
  });
  ...
  if (verdict === 'low_risk' && !hasCritical) {
    steps.push({
      action: 'safe_to_proceed',
      priority: 'low',
      description: 'No critical flags were detected. Proceed with normal professional caution and independent verification.',
    });
  }
